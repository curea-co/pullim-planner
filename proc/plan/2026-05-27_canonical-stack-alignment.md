...
> ⚠ **순서 정정**: P1 전체가 "P0 완료 후"는 아니다. §13-A·§14-A 대로 **P1-1(JWT)·P1-2(Redis/BullMQ)의 코드 레벨 작업(9c/10c)** 은 인프라 보류와 분리되어 (각 리포 scope-out 해제 후) **선행 가능**하다. 반면 **인프라 의존 배포 단계(P1-1/P1-2 의 배포 측면, §13-B 9/10)는 P0(특히 §16.3 트리거) 이후**다. P1-3/4/5(FE) 는 **FE 게이트키퍼 G4(§12)** + 권위 문서 갱신 후. (여기서 G4 는 §12 게이트키퍼이며, §4 의 갭 G4(BE 프레임워크)와 무관하다.) 아래 표는 산출물 정의이며 진입 순서는 §13/§14 를 따른다.
...
> ⚠ **명명 충돌 주의 — 게이트 ≠ 갭 ID**: 본 §12 의 `G1/G3/G4` 는 **게이트키퍼(승인자)** 다. §4 의 `G1~G15` 는 **갭 분석 번호**(예: §4 `G4` = BE 프레임워크 갭)로 전혀 다른 의미다. 혼동을 막기 위해, 본 문서에서 **승인 게이트를 가리킬 때는 항상 "게이트키퍼 G_" 또는 "G_ 게이트"** 로 적고, §4 갭을 가리킬 때는 "갭 G_" 로 적는다. (예: "P1-3/4/5 는 FE 게이트키퍼 G4 후" ↔ "갭 G4 = BE 프레임워크".)
...
| 9c | P1-1(코드) | 4 도메인(games 제외) | `feat(<scope>): JWT 인증 **코드 레벨** 구현 (로컬 docker 검증)` | 각 리포 scope-out 해제(§14-A) — 인프라 비의존. ⚠ **games 는 BE 부재(D-GM-BE) — BE skeleton/모듈 신설 완료 후에만 착수**(별도 선행). **배포 측면(PR9)은 13-B** |
| 10c | P1-2(코드) | 4 도메인(games 제외) | `feat(<scope>): Redis/BullMQ **코드 레벨** 구현 (로컬 docker 검증)` | 각 리포 scope-out 해제(§14-A) — 인프라 비의존. ⚠ **games 는 BE 신설 완료 후**(9c 와 동일). **배포 측면(PR10)은 13-B** |
...
- [ ] **(14-A 에서 이관 — D-DS 해제 후)** 5 도메인 모두 `@pullim/design-system` 사용 — 본체팀 외부 노출 정책(D-DS) 합의 후 마이그레이션. D-DS 가 풀리지 않으면 본 항목·전체 최종 완료가 닫히지 않는다.
...
> ⚠ **복제 시 경로 처리 방침 (권위 문서로 기능하기 위한 필수 절차)**: 본 사본은 **planner 파생본**이며, 본문의 planner 전용 상대경로는 planner 리포에서만 유효하다. §0/부록 A 는 "권위 문서를 실제로 따라갈 수 있어야 한다"를 전제하므로, **다른 리포로 복제하는 순간 경로를 치환하지 않으면 그 전제가 깨진다.** 따라서 복제는 단순 copy 가 아니라 아래 **치환 체크리스트를 반드시 수행한 파생본 생성**으로 정의한다:
>
> | 템플릿 변수 | planner 값 | 복제 시 치환 |
> |---|---|---|
> | `{앱가이드-AGENTS}` | `apps/planner/AGENTS.md` | 대상 리포의 앱 가이드 경로 (없으면 루트 `AGENTS.md`) |
> | `{앱가이드-CLAUDE}` | `apps/planner/CLAUDE.md` | 대상 리포의 앱 가이드 경로 |
> | `{BE채택plan}` | `proc/plan/2026-05-26_pullim-be-adoption.md` | 대상 리포의 BE 채택 plan (없으면 "해당 없음" 명시) |
> | `{FE채택plan}` | `proc/plan/2026-05-26_container-presenter-adoption.md` | 대상 리포의 FE 채택 plan |
> | `{권위핸드오프}` | `input/docs-archive/08_풀림_플래너_핸드오프.md` | 대상 리포의 권위 문서 (§3 매트릭스 '권위 문서' 열 참조) |
>
> 복제 PR 은 위 표의 모든 변수를 대상 리포 실제 경로로 치환했는지 확인하고, 치환 불가(대상 리포에 대응 문서 부재) 항목은 본문에 "해당 없음"으로 명시한다. 치환 전 원본을 그대로 머지하면 안 된다. **본 planner 사본은 위 'planner 값' 열이 정본이다.**
