...
| 구현 범위(출시) | ⚠️ **출시(06-29)=FE 화면설계(mock)만.** 실 BE(pullim-api 엔드포인트·마이그레이션)는 **2026-06-30 착수**(사용자 확정 2026-06-24). 출시일엔 pullim-api 팀 의존+공유 DB 마이그레이션이라 불가. 자세히는 §12 |
...
| **실 BE(06-30 착수)** | **pullim-api 루틴 엔드포인트 + Routine 엔티티 + 마이그레이션** + materialize + api-client 실연동 | **2026-06-30** (R0~R3) |

출시 FE 일정: R4·R5 = ~1.5 dev-day → 06-29 여유. OI 3건 결정(§11)은 **실 BE 설계**에 그대로 사용.

### 12.5 실 BE 트랙 — **2026-06-30 착수** (사용자 확정 2026-06-24)
- **타깃**: pullim-api 루틴 엔드포인트 + Routine 엔티티 + **공유 DB 마이그레이션** (이 repo `apps/backend` 아님 — FE가 라이브로 무는 곳).
- **시작점**: 본 명세를 **pullim-api 요구사항 핸드오프**로 변환 → `pullim-api/docs/planner/`에 cutover와 동일 양식으로 요청(엔티티·엔드포인트·마이그레이션·materialize). §4 비구속 초안의 미정 계약 5건(소유단위·subject taxonomy·overlap·suppression·완료모델)을 핸드오프에서 확정.
- **게이트**: 게이트키퍼와 마이그레이션 배포창 협의. R0(계약 타입, api-client) → R3(FE 실연동)은 BE 배포에 맞춰 켠다. prod 노출은 실 BE + 게이트 뒤(§9).
