...
- **OI-1 · 루틴 소유 단위** — ✅ **결정(2026-06-24): 플래너 단위(`plannerId` FK).** 시험별 일과가 다름. 추후 "모든 시험 공통 생활 루틴" 요구가 크면 userId + 적용범위 옵션으로 확장.
- **OI-2 · materialize 시점** — ✅ **결정(2026-06-24): 조회 시 on-the-fly 합성(DB 저장 안 함).** 완료 상태·통계가 실체 블록을 요구하면 **"완료 시 승격(promote)"** 하이브리드(OI-4 연계).
