...
> ¹·⁴ 는 **게이트키퍼 원표의 각주 표기를 그대로 옮긴 것** — 원문 각주 본문은 이 문서에 전달되지 않았다.
> 문맥상 ¹ = 비로그인(게스트) 상태, ⁴ = 수원 학생 쿠폰 grant 관련으로 추정되며, ⁴ 가 실제로 무엇을
> 설명하는지는 아래 "§유의" 의 "각주 ⁴ 확인 권장" 항목과 직결된다.

**결론: 로그인 화면 "표시" 기준으로는 4칸 모두 실제 구현과 일치.** (검증일 기준)
단, 표시가 같아도 **수원 학생의 실 유효 등급은 다르다**(planner:2) — 아래 §유의 참조.
이 문서는 표시 검증 기록이며, 엔타이틀먼트 등급 체계 자체의 검증서가 아니다.
