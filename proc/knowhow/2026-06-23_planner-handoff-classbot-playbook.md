...
플래너의 개발 PR·통합 승격 적용 범위는 [루트 CLAUDE.md의 최상위 규칙](../../CLAUDE.md)을 따른다(2026-09-08 오너 결정). 아래 분리 원칙은 **기능 개발 PR(feature → dev)** 에 적용한다. 검증된 dev 상태의 단일 배포 목적을 갖는 통합 승격은 소유 규칙의 검증 근거·최신 head 통합 회귀·검토 가능한 범위 조건을 충족해야 한다. 조건을 충족하지 못하면 범위를 줄이거나 단계별로 나누고, 승격의 코드·보안·CI·리뷰 게이트는 유지한다.
