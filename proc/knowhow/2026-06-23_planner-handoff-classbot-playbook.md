...
PR 범위의 정본은 [루트 CLAUDE.md 의 최상위 규칙](../../CLAUDE.md)이다 — **모든 PR은 한 관심사**이고, `head=dev, base=main` 승격 PR 의 관심사는 「검증된 dev 상태를 main 에 반영한다」 하나로 정의된다. 아래 분리 원칙은 그 규칙을 개발 PR 에 적용한 실무 형태이며, 승격의 검증 근거·최신 head 통합 회귀·검토 가능한 범위 조건은 정본에서 확인한다. 여기 적힌 것과 정본이 어긋나면 정본이 이긴다.
