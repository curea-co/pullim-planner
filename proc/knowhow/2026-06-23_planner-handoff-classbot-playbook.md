...
> `proc/plan/2026-06-23_pullim-ds-pr1-foundation.md` 등 **내부 링크는 클래스봇 레포 기준 경로**이며,
> 이 플래너 레포에는 존재하지 않는다(열리지 않음). 패턴·접근만 참고하고, 실제 파일은 클래스봇 레포에서 확인할 것.
...
플래너의 개발 PR·통합 승격 적용 범위는 [루트 CLAUDE.md의 최상위 규칙](../../CLAUDE.md)을 따른다(2026-09-08 오너 결정). 아래 분리 원칙은 **일반 PR(기능·버그 수정·문서·운영 변경 등)** 에 종류·대상 브랜치와 무관하게 적용한다. head=dev, base=main인 승격만 소유 규칙의 별도 조건을 따른다. 검증된 dev 상태의 단일 배포 목적을 갖는 통합 승격은 소유 규칙의 검증 근거·최신 head 통합 회귀·검토 가능한 범위 조건을 충족해야 한다. 조건을 충족하지 못하면 범위를 줄이거나 단계별로 나누고, 승격의 코드·보안·CI·리뷰 게이트는 유지한다.
