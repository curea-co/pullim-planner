...
export function createPullimSessionClient(
   ...
   * 이런 순서가 생긴다:
   *
   * 1. R1..R38 이 만료 직전 액세스 토큰으로 동시에 나간다
   * 2. R1 이 401 → 재발급 시작 → R2..R20 은 같은 promise 를 기다린다 (여기까진 의도대로)
   * 3. 재발급 성공. 쿠키가 새것으로 갈렸고 `refreshInFlight` 는 null 이 된다
   * 4. **R21 의 401 이 이제서야 도착한다** — 만료된 옛 토큰으로 보낸 요청이니 당연히 401 이다
   * 5. `refreshInFlight` 가 null 이라 R21 이 **2차 재발급**을 시작한다
   *
   * 5번이 문제다. 리프레시 토큰은 회전(rotation)하므로 2차 재발급은 **이미 쓴 토큰을 다시
   * 내미는 꼴**이고, 서버의 재사용 유예를 벗어나면 거부된다. 그러면 R21 의 원 401 이 그대로
   * 올라가 `on401` → 세션 만료 전파 → **앱 전체가 로그아웃**된다. 실제로는 로그인이 멀쩡한데도.
   *
   * 그런데 3번에서 쿠키는 이미 새것이 됐다 — **R21 은 재발급이 필요 없고 재시도만 하면 된다.**
   * 그래서 재발급 직후 짧은 창 안의 요청에는 재발급을 다시 하지 않고 성공만 돌려준다.
   * 호출부(`cookie-http`)는 그 값을 받아 새 쿠키로 원 요청을 1회 재시도한다.
   *
   * ## 창 길이를 5초로 잡은 근거
   *
   * 막으려는 것은 **같은 버스트의 늦은 응답**이다. 한 화면이 쏘는 수십 개의 응답이 흩어지는
   * 폭이 그 정도면 충분하고, 액세스 토큰 수명(분 단위)보다는 두 자릿수 배 짧다. 즉 이 창
   * 안에서 토큰이 *진짜로* 다시 만료되는 일은 없다.
   *
   * 만에 하나 창 안의 401 이 진짜였다면(재발급으로도 못 고칠 사유), 재시도가 401 로 끝나
   * 지금과 **같은 결과**가 된다 — 이 변경이 새로 만드는 실패는 없다.
   *
   * ## 왜 정확한 방법(세대 카운터)을 안 썼나
   *
   * "요청이 시작된 뒤에 재발급이 성공했는가"를 보면 시간 추정 없이 정확히 판정할 수 있다.
   * 다만 그러려면 요청 시작 시점을 `cookie-http` 에서 `refreshSession` 까지 넘겨야 해서
   * **공유 fetch 래퍼의 계약**이 바뀐다. 얻는 정확도에 비해 번지는 범위가 커서 택하지 않았다.
   * 이 창은 그 계약을 건드리지 않고 이 파일 안에서 닫힌다.
   */
  let lastRefreshOkAt = 0;
  /** 재발급 직후 「쿠키가 이미 새것」이라고 볼 창. 위 주석의 근거 참조. */
  const REFRESH_REUSE_WINDOW_MS = 5_000;
  function refreshSession(): Promise<boolean> {
    if (!refreshInFlight) {
      // 방금 재발급됐다면 다시 하지 않는다 — 회전된 토큰을 재제출해 거부당하는 것이
      // 이 분기가 막으려는 유일한 사고다.
      if (Date.now() - lastRefreshOkAt < REFRESH_REUSE_WINDOW_MS) {
