...
> 적용 시점: 이 개정이 포함된 base revision을 기준으로 하는 리뷰부터 적용한다. 규칙을 개정하는 PR 자체에 새 예외를 소급 적용하지 않는다. 오너가 요청한 #265는 기존 규칙의 단일 관심사에 맞춘 규칙 문서 전용 PR이며, 기능 통합 변경을 포함하지 않는다.

- **일반 PR은 종류·대상 브랜치와 무관하게 한 PR = 한 관심사.** 기능·버그 수정·문서·운영 변경을 모두 포함하며, 독립적으로 구현·검증할 수 있는 변경은 작은 PR로 분리한다. 별도 적용 조건은 아래 head=dev, base=main 승격에만 둔다.
- **승격 PR(head=dev, base=main)은 검증된 dev 상태를 main으로 반영하는 하나의 배포 목적에 한해 통합으로 올릴 수 있다.** 포함 변경은 관심사별 개발 PR에서 각각 독립적으로 동작·검증되어 dev에 머지된 것이어야 한다. 승격 과정에서 미검증 기능이나 별도 목적의 변경을 추가하지 않는다.
- **검증 근거를 추적할 수 있어야 한다.** 승격 PR 본문에 포함 개발 PR 목록, 각 PR의 검증된 head SHA와 CI·리뷰 결과, 승격 대상 dev head SHA를 기록한다. 개발 PR의 검증만으로 통합 결과를 대신하지 않으며, 승격 최신 head에서도 전체 CI·통합 회귀 검증을 통과해야 한다.
- **승격도 검토 가능한 범위를 유지한다.** 포함 PR과 누적 diff를 대조하여 실제 코드 결함·보안·계약 정합성 및 변경 간 상호작용을 검토한다. 독립 검증 근거가 없거나 통합 영향이 불명확하거나 한 번의 리뷰에서 검토·수렴할 수 없는 범위이면, 근거를 보완하고 승격 범위를 줄이거나 단계별 승격으로 나눈 뒤 다시 검증한다.
