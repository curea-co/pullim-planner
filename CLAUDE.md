...
- **모든 PR은 한 PR = 한 관심사.** 종류·대상 브랜치와 무관하다 — 기능·버그 수정·문서·운영 변경 전부.
  독립적으로 구현·검증할 수 있는 변경은 작은 PR로 분리한다. diff 가 **Codex Review 가 한 번에
  탐지·수렴할 수 있는 depth 를 초과**하면 리뷰가 무한 반복(COMMENTED 누적)되어 머지가 끝나지 않는다.
...
- **오너 확인 이력** — 이 절은 § 수정 금지 영역의 컨벤션 문서다. 고칠 때는 **오너 확인 + 컨벤션 전용 PR**
  ...
  - **2026-09-09** — 위 개정을 dev 에도 같은 문구로 정합하도록 확인(“dev 정합 진행해”).
    이 항목의 확인 근거는 **이 줄 자체가 아니라 main 의 머지 기록**이다 — 여기서 새로 정하는
    규칙 문구가 하나도 없기 때문이다. 위 제목과 불릿 5개는 오너 확인·리뷰·머지를 이미 거친
    main 의 본문(PR #265 · 머지 커밋 `e640af3`)과 **바이트 동일**하고, 이 PR 이 더하는 것은
    이 이력 항목 하나뿐이다. 그래서 `git show e640af3:CLAUDE.md` 와 대조하면 확인 여부를
    이 문서 바깥에서 검증할 수 있다.
